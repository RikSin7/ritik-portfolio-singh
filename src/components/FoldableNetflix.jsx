...
import netflix from "../assets/images/NetflixHomeSs.png";
import {
  motion,
  useMotionValue,
  useMotionValueEvent,
...
function FoldableNetflix({ setIsHovering, isOpenNetflix, setIsOpenNetflix }) {
  ...
  useMotionValueEvent(xDrag, "change", (currentX) => {
    if (currentX > 260) {
      setIsFolded(false);
      setIsOpenNetflix(true);
    } else {
      setIsFolded(true);
      setIsOpenNetflix(false);
      setIsClicked(false);
    }
  });
        ...
        >
          ...
          <motion.div
            className={`map-image bg-[size:300%] translate-x-full ${
              isOpenNetflix && isClicked && netflix && "border-none rounded-l-2xl"
            } border-r border-black shadow-2xl shadow-black`}
            style={{
              backgroundImage: `${
                isOpenNetflix && isClicked && netflix
                  ? `url(${netflix})` // No gradient when currConv
          ...
          <motion.div
            className={`map-image bg-[size:300%] bg-right -translate-x-full ${
              isOpenNetflix && isClicked && netflix && "border-none rounded-r-2xl"
...
export default FoldableNetflix;
